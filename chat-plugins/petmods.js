...
evgutter: function (target, room, user) {
	...
	datalistool: function (target, room, user) {
		...
		Object.values(feDex).forEach(mon => {
			...
			buf += `if (fuse1 === '${mon.species}') {<br>document.getElementById("typearea").innerHTML = "&lt;img src="http://play.pokemonshowdown.com/sprites/types/${mon.types[0]}.png&gt;"<br>}<br>`;
			}
			else {
					buf += `if (fuse1 === '${mon.species}') {<br>document.getElementById("typearea").innerHTML = "&lt;img src="http://play.pokemonshowdown.com/sprites/types/${mon.types[0]}.png"&gt;&lt;img src="http://play.pokemonshowdown.com/sprites/types/${mon.types[1]}.png&gt;"<br>}<br>`;
			}	 
		}
		);
		this.sendReplyBox(`${buf}</div>`);
