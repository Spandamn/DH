...
learnistor: function(target, room, user) {
	...
	eeveed2: function (target, room, user) {
		...
		if (!jillianDex) return this.errorReply("Error Fetching Istor Data.");
		Object.values(jillianDex).forEach(mon => {
			buf += `<table border=1><tr><td>${mon.species}</td><td>${mon.types}</td><td>${mon.baseStats.hp}</td><td>${mon.baseStats.atk}</td><td>${mon.baseStats.def}</td><td>${mon.baseStats.spa}</td><td>${mon.baseStats.spd}</td><td>${mon.baseStats.spe}</td></tr></table>`;
		});
		this.sendReplyBox(`${buf}</div>`);
	},
