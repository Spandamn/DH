...
let BattleMovedex = {
	...
	"healingterrain": {
		...
		shortDesc: "5 turns. Grounded: -Dark priority moves,+1/3 max HP on switch-out.",
		id: "healingterrain",
	...
	"fieldreset": {
		...
		flags: {protect: 1, reflectable: 1, mirror: 1, authentic: 1},
		onHit: function (target, source, move) {
			let removeTarget = {spikes:1, toxicspikes:1, stealthrock:1, stickyweb:1, reflect:1, lightscreen:1, auroraveil: 1, safeguard:1, mist:1};
			let removeAll = {spikes:1, toxicspikes:1, stealthrock:1, stickyweb:1, reflect:1, lightscreen:1, auroraveil: 1, safeguard:1, mist:1};
	...
	"timeout": {
		...
		category: "Status",
		desc: "If this move is successful and the user has not fainted, the user switches out even if it is trapped and is replaced immediately by a selected party member. The user does not switch out if there are no unfainted party members, or if the target switched out using an Eject Button.",
		shortDesc: "User switches out after damaging the target and heals 1/3 of it's HP.",
		id: "timeout",
		isViable: true,
		name: "Time-Out",
		pp: 20,
	...
	"heatdrain": {
		...
		category: "Special",
		desc: "The user recovers 1/2 the HP lost by the target, rounded half up. If Big Root is held by the user, the HP recovered is 1.3x normal, rounded half down.",
		shortDesc: "User recovers 50% of the damage dealt. Foe cant use Fire moves.",
		id: "heatdrain",
		isViable: true,
		name: "Heat Drain",
		pp: 10,
		...
		flags: {protect: 1, mirror: 1, heal: 1},
		drain: [1, 2],
		volatileStatus: 'heatdrain',
		effect: {
			duration: 10000000000,
			onStart: function (pokemon) {
				this.add('-start', pokemon, 'move: Heat Drain');
			},
			onDisableMove: function (pokemon) {
				for (const moveSlot of pokemon.moveSlots) {
					if (this.getMove(moveSlot.id).type === 'Fire') {
						pokemon.disableMove(moveSlot.id);
					}
				}
			},
			...
			onBeforeMove: function (pokemon, target, move) {
				if (move.type === 'Fire') {
					this.add('cant', pokemon, 'move: Heat Drain', move);
					return false;
				}
			},
			onResidualOrder: 17,
			onEnd: function (pokemon) {
				this.add('-end', pokemon, 'move: Heat Drain');
			},
		},
		target: "normal",
		type: "Space",
