	...
	weatherbreak: { // https://hastebin.com/emuxidukok.pas
		...
		onBasePower: function (basePower, attacker, defender, move, effect) {
			if (!attacker.volatiles['atmosphericperversion']){
				if (this.isWeather('sunnyday') && move.type === 'Fire') return this.chainModify([0x0547, 0x1000]);
				...
				if (this.isWeather('raindance') && move.type === 'Water') return this.chainModify([0x0547, 0x1000]);
				...
				if (this.isWeather('shadowdance') && move.type === 'Ghost') return this.chainModify([0x0547, 0x1000]);
			}
		},
	},
